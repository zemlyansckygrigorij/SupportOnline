...
public class Settings {
    ...
    public static void writeClaim(String claim){
        File f = new File(System.getProperty("user.dir") +"\\ListClaim.txt");
        if(!f.exists()){
            try {
                f.createNewFile();
            } catch (IOException e) {
        ...
        try {
            Files.write(Paths.get(System.getProperty("user.dir") +"\\ListClaim.txt"), claim.getBytes(), StandardOpenOption.APPEND);
        }
        catch (IOException e) {
            JFrame myWindow = new FrameException("Ошибка записи в файл !!!");
            Settings.writeError(e);
